...
function leonardo_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'leonardo' ),
		)
	);
...
function leonardo_scripts() {
	wp_enqueue_style( 'leonardo-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'leonardo-style', 'rtl', 'replace' );

	wp_enqueue_script( 'leonardo-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function register_widget_areas() {
	...
	register_sidebar( array(
	  ...
	  'id'            => 'footer_area_five',
	  'description'   => 'This widget area discription',
	  'before_widget' => '<section class="footer-area foot-five">',
	  'after_widget'  => '</section>',
	  'before_title'  => '<p>',
	  'after_title'   => '</p>',
	));
  ...
  register_nav_menus(
	array(
		'menu-1' => esc_html__( 'MAin head', 'leonardo' ),
